...
import { DeviceKeyboard } from '../../components/DeviceKeyboard';
...
export const CalculatorMainView = React.memo<CalculatorMainViewProps>(function CalculatorMainView({
  ...
  // Focus the "You send" input on mount
  useEffect(() => {
    if (!hasBeenFocused) {
      onFromFocus();
  ...
  const handleKeyPress = (key: string) => {
    const currentValue = activeField === 'from' ? (fromAmount || '0') : (toAmount || '0');
    ...
    if (activeField === 'from') {
      onFromAmountChange(newValue);
    } else {
      onToAmountChange(newValue);
    }
  };
  ...
  // Handle input blur - don't do anything, keyboard stays open
  const handleInputBlur = () => {
    // Don't hide keyboard on blur - only hide via Done button
  };

  // Handle done - hide keyboard and blur input
  const handleDone = () => {
    if (activeField === 'from') {
      onFromBlur();
    } else {
  ...
  return (
    <motion.div
      ...
      exit={getExitVariant()}
    >
      <div className="px-4 py-4 flex justify-between items-center flex-shrink-0">
        {onBack && (
          <Button
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3" style={{ paddingBottom: showKeyboard ? '380px' : '100px' }}>
        <div className="space-y-4">
          ...
          >
            <SendReceiveInput
              label={recipientName ? `${recipientName} gets` : "Recipient gets"}
              value={toAmount}
      ...
      <div className="fixed bottom-0 left-0 right-0 px-4 pt-4 pb-12 bg-wise-background-screen" style={{ zIndex: 20 }}>
        <Button
          variant="primary"
          size="large"
